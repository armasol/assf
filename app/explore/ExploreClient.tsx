...
export function ExploreClient({ tokens }: { tokens: Token[] }) {
  ...
  return <div className="page-wrap content-page">
    ...
    <Reveal><div className="stats-grid four"><StatCard label="TOKENS ON PONS" value={String(tokens.length)} note="from Supabase"/><StatCard label="CREATORS LINKED" value={String(new Set(tokens.map((token) => token.creator)).size)} note="from recorded launches"/><StatCard label="TRADE VOLUME" value="Unavailable" note="no trade ledger connected" accent/><StatCard label="TOP MOVER" value="Unavailable" note="no market data connected"/></div></Reveal>
