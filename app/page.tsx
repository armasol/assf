...
export default async function HomePage() {
  ...
  const displayedTokens = liveTokens.length ? liveTokens : tokens;
  const displayedReceipts = launches?.length
  ...
  return <div className="home-page page-wrap">
    ...
    <Reveal><section className="home-section"><SectionTitle title="Top tokens" link="Explore all"/><div className="token-grid four">{displayedTokens.slice(0,4).map(t => <TokenCard key={t.ticker} token={t}/>)}</div></section></Reveal>
    ...
    <Reveal><section className="split-section home-section">
      ...
      <div><SectionTitle title="Recent creator fees" link="Payments" href="/payments"/><div className="receipt-list compact">{displayedReceipts.slice(0,4).map(r=><div className="receipt-row" key={r.id}><div className="gift-icon"><GiftIcon size={17}/></div><div><strong>{r.handle}</strong><span>{r.time}</span></div><strong>{r.amount}</strong></div>)}</div></div>
    </section></Reveal>
